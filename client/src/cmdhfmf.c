...
static int CmdHF14AMfBLGen(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "hf mfu blgen",
                  "Generate keys for a Bambu Lab filament tag",
                  "hf mfu blgen -r\n"
                  "hf mfu blgen -r -d\n"
                  "hf mfu blgen --u 11223344\n"
                 );
...
//    {"keybrute",    CmdHF14AMfKeyBrute,     IfPm3Iso14443a,  "J_Run's 2nd phase of multiple sector nested authentication key recovery"},
    ...
    {"blgen",       CmdHF14AMfBLGen,          AlwaysAvailable, "Generate key table for Bambu Lab filament tag"},
